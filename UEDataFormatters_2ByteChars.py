...
def find_first_fname(sbval, depth=0, max_depth=15):
    if not sbval.IsValid() or depth > max_depth:
        return None

    type_name = sbval.GetTypeName() or ""
    if "FName" in type_name:
        return sbval

    for child in sbval:  # Direct iteration over children
        found = find_first_fname(child, depth + 1, max_depth)
        if found:
            return found

    return None
def read_nameprivate(sbval):
    target = lldb.debugger.GetSelectedTarget()
    fname_type = target.FindFirstType("FName")

    # Get pointer to object
    if sbval.TypeIsPointerType():
        addr = sbval.GetValueAsUnsigned()
    else:
        addr = sbval.AddressOf().GetValueAsUnsigned()

    # NamePrivate is at offset 0x18 in UE5.6 (64-bit)
    fname_addr = addr + 0x18
    sbaddr = lldb.SBAddress(fname_addr, target)
    fname_val = target.CreateValueFromAddress("NamePrivate", sbaddr, fname_type)
    return fname_val.GetSummary() or "<no summary>"

def UEUObjectBaseSummaryProvider(valobj, internal_dict):
    #Name = valobj.GetChildMemberWithName('NamePrivate')
    Name = read_nameprivate(valobj)
    return Name
